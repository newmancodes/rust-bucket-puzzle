...
impl Bucket {
    pub fn new(label: &str, capacity: u8) -> Self {
        Bucket {
...
mod tests {
    ...
    fn a_bucket_is_created_empty() {
        ...
        // Act
        let bucket = Bucket::new(label, capacity);

        // Assert
        assert_eq!(bucket.get_label(), label);
    ...
    fn a_bucket_can_be_filled() {
        // Arrange
        let bucket = Bucket::new("some_bucket_label", 5);

        // Act
        let filled_bucket = bucket.fill();

        // Assert
        assert_eq!(filled_bucket.get_label(), "some_bucket_label");
    ...
    fn a_bucket_can_be_emptied() {
        // Arrange
        let bucket = Bucket::new("some_bucket_label", 45).fill();

        // Act
        let emptied_bucket = bucket.empty();

        // Assert
        assert_eq!(emptied_bucket.get_label(), "some_bucket_label");
    ...
    fn a_bucket_can_be_poured_into() {
        // Arrange
        let source_bucket = Bucket::new("source_bucket", 30).fill();
        let target_bucket = Bucket::new("target_bucket", 25);

        // Act
        let (filled_target_bucket, reduced_source_bucket) = target_bucket.pour_into(source_bucket);

        // Assert
        assert_eq!(reduced_source_bucket.get_label(), "source_bucket");
        assert_eq!(reduced_source_bucket.get_capacity(), 30);
        assert_eq!(reduced_source_bucket.get_current_volume(), 5);

        assert_eq!(filled_target_bucket.get_label(), "target_bucket");
